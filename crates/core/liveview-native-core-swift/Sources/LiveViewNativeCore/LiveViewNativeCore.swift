...
public struct FfiConverterTypeLiveSocketError: FfiConverterRustBuffer {
    ...
    public static func read(from buf: inout (data: Data, offset: Data.Index)) throws -> LiveSocketError {
        let variant: Int32 = try readInt(&buf)
        switch variant {
        case 1: return try .InvalidHeader(
                error: FfiConverterString.read(from: &buf)
            )
        case 2: return try .InvalidMethod(
                error: FfiConverterString.read(from: &buf)
            )
        case 3: return try .Phoenix(
                error: FfiConverterString.read(from: &buf)
            )
        case 4: return try .Request(
                error: FfiConverterString.read(from: &buf)
            )
        case 5: return try .Parse(
                error: FfiConverterTypeParseError.read(from: &buf)
            )
        case 6: return try .JsonDeserialization(
                error: FfiConverterString.read(from: &buf)
            )
        case 7: return .CsfrTokenMissing
        case 8: return .PhoenixIdMissing
        case 9: return .PhoenixSessionMissing
        case 10: return .PhoenixStaticMissing
        case 11: return .PhoenixMainMissing
        case 12: return .NoHostInUrl
        case 13: return .NoUploadToken
        case 14: return .NoLiveReloadUrl
        case 15: return try .SchemeNotSupported(
                scheme: FfiConverterString.read(from: &buf)
            )
        case 16: return try .Upload(
                error: FfiConverterTypeUploadError.read(from: &buf)
            )
        case 17: return .NoDocumentInJoinPayload
        case 18: return try .DocumentMerge(
                error: FfiConverterTypeMergeError.read(from: &buf)
            )
        case 19: return try .DocumentRender(
                error: FfiConverterTypeRenderError.read(from: &buf)
            )
        case 20: return .NoInputRefInDocument
        case 21: return try .Serde(
                error: FfiConverterString.read(from: &buf)
            )
        case 22: return try .Events(
                error: FfiConverterString.read(from: &buf)
            )
        default: throw UniffiInternalError.unexpectedEnumCase
        }
