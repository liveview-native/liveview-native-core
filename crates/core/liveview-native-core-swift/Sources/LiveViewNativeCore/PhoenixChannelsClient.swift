    ...
    SocketProtocol
{
     ...
     */
    public static func spawn(url: Url, cookies: [String]?, reconnectStrategy: ReconnectStrategy?) async throws -> Socket {
        return
            try await uniffiRustCallAsync(
                rustFutureFunc: {
                    uniffi_phoenix_channels_client_fn_constructor_socket_spawn(FfiConverterTypeUrl.lower(url), FfiConverterOptionSequenceString.lower(cookies), FfiConverterOptionCallbackInterfaceReconnectStrategy.lower(reconnectStrategy))
                },
                pollFunc: ffi_phoenix_channels_client_rust_future_poll_pointer,
                completeFunc: ffi_phoenix_channels_client_rust_future_complete_pointer,
                freeFunc: ffi_phoenix_channels_client_rust_future_free_pointer,
 ...
 */
public protocol ReconnectStrategy: AnyObject {
    /**
     ...
     */
    func sleepDuration(attempt: UInt64) -> TimeInterval
}
...
private enum UniffiCallbackInterfaceReconnectStrategy {
    // Create the VTable using a series of closures.
    // Swift automatically converts these into C callback functions.
    static var vtable: UniffiVTableCallbackInterfaceReconnectStrategy = .init(
        sleepDuration: { (
            uniffiHandle: UInt64,
            attempt: UInt64,
            ...
            let makeCall = {
                ...
                guard let uniffiObj = try? FfiConverterCallbackInterfaceReconnectStrategy.handleMap.get(handle: uniffiHandle) else {
                    throw UniffiInternalError.unexpectedStaleHandle
                }
                return try uniffiObj.sleepDuration(
                    attempt: FfiConverterUInt64.lift(attempt)
                )
            }
        ...
        uniffiFree: { (uniffiHandle: UInt64) in
            let result = try? FfiConverterCallbackInterfaceReconnectStrategy.handleMap.remove(handle: uniffiHandle)
            if result == nil {
                print("Uniffi callback interface ReconnectStrategy: handle missing in uniffiFree")
            }
        }
    )
}

private func uniffiCallbackInitReconnectStrategy() {
    uniffi_phoenix_channels_client_fn_init_callback_vtable_reconnectstrategy(&UniffiCallbackInterfaceReconnectStrategy.vtable)
}
...
// FfiConverter protocol for callback interfaces
    ...
    @_documentation(visibility: private)
#endif
private enum FfiConverterCallbackInterfaceReconnectStrategy {
    fileprivate static var handleMap = UniffiHandleMap<ReconnectStrategy>()
}

#if swift(>=5.8)
    @_documentation(visibility: private)
#endif
extension FfiConverterCallbackInterfaceReconnectStrategy: FfiConverter {
    typealias SwiftType = ReconnectStrategy
    typealias FfiType = UInt64
...
private struct FfiConverterOptionTypePayload: FfiConverterRustBuffer {
    ...
    @_documentation(visibility: private)
#endif
private struct FfiConverterOptionCallbackInterfaceReconnectStrategy: FfiConverterRustBuffer {
    typealias SwiftType = ReconnectStrategy?
    ...
    public static func write(_ value: SwiftType, into buf: inout [UInt8]) {
        ...
        writeInt(&buf, Int8(1))
        FfiConverterCallbackInterfaceReconnectStrategy.write(value, into: &buf)
    }

    public static func read(from buf: inout (data: Data, offset: Data.Index)) throws -> SwiftType {
        switch try readInt(&buf) as Int8 {
        case 0: return nil
        case 1: return try FfiConverterCallbackInterfaceReconnectStrategy.read(from: &buf)
        default: throw UniffiInternalError.unexpectedOptionalTag
        }
    }
}
...
private var initializationResult: InitializationResult = {
    ...
    if uniffi_phoenix_channels_client_checksum_constructor_socket_spawn() != 34812 {
        return InitializationResult.apiChecksumMismatch
    }
    if uniffi_phoenix_channels_client_checksum_constructor_topic_from_string() != 28533 {
        return InitializationResult.apiChecksumMismatch
    }
    if uniffi_phoenix_channels_client_checksum_method_reconnectstrategy_sleep_duration() != 52540 {
        return InitializationResult.apiChecksumMismatch
    }

    uniffiCallbackInitReconnectStrategy()
