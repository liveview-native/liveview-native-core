...
private struct _JsonKeyedEncodingContainer<Key: CodingKey>: KeyedEncodingContainerProtocol {
    ...
    public mutating func superEncoder() -> Encoder {
        return __JsonReferencingEncoder(
            referencing: self.encoder, key: AnyCodingKey(stringValue: "super"),
            convertedKey: "super", codingPath: self.encoder.codingPath, wrapping: self.reference)
    }

    public mutating func superEncoder(forKey key: Key) -> Encoder {
        return __JsonReferencingEncoder(
            referencing: self.encoder, key: key, convertedKey: key.stringValue,
            codingPath: self.encoder.codingPath, wrapping: self.reference)
    }
}
...
private struct _JsonUnkeyedEncodingContainer: UnkeyedEncodingContainer {
    ...
    public mutating func encode(_ value: Bool) throws {
        self.reference.insert(self.encoder.wrap(value))
    }
    ...
    public mutating func superEncoder() -> Encoder {
        return __JsonReferencingEncoder(
            referencing: self.encoder, at: self.reference.count,
            codingPath: self.encoder.codingPath, wrapping: self.reference)
    }
}
...
extension __JsonEncoder {
    @inline(__always) fileprivate func wrap(_ value: Bool) -> JsonReference {
        .init(.bool(bool: value))
    }
    @inline(__always) fileprivate func wrap(_ value: Int) -> JsonReference {
        if value >= 0 {
    ...
    @inline(__always) fileprivate func wrap(_ value: Int8) -> JsonReference {
        if value >= 0 {
            .init(.numb(number: .posInt(pos: UInt64(value))))
        } else {
            .init(.numb(number: .negInt(neg: Int64(value))))
    ...
    @inline(__always) fileprivate func wrap(_ value: Int16) -> JsonReference {
        if value >= 0 {
            .init(.numb(number: .posInt(pos: UInt64(value))))
        } else {
            .init(.numb(number: .negInt(neg: Int64(value))))
    ...
    @inline(__always) fileprivate func wrap(_ value: Int32) -> JsonReference {
        if value >= 0 {
            .init(.numb(number: .posInt(pos: UInt64(value))))
        } else {
            .init(.numb(number: .negInt(neg: Int64(value))))
    ...
    @inline(__always) fileprivate func wrap(_ value: Int64) -> JsonReference {
        if value >= 0 {
            .init(.numb(number: .posInt(pos: UInt64(value))))
        } else {
            .init(.numb(number: .negInt(neg: Int64(value))))
    ...
    @inline(__always) fileprivate func wrap(_ value: UInt) -> JsonReference {
        .init(.numb(number: .posInt(pos: UInt64(value))))
    }
    @inline(__always) fileprivate func wrap(_ value: UInt8) -> JsonReference {
        .init(.numb(number: .posInt(pos: UInt64(value))))
    }
    @inline(__always) fileprivate func wrap(_ value: UInt16) -> JsonReference {
        .init(.numb(number: .posInt(pos: UInt64(value))))
    }
    @inline(__always) fileprivate func wrap(_ value: UInt32) -> JsonReference {
        .init(.numb(number: .posInt(pos: UInt64(value))))
    }
    @inline(__always) fileprivate func wrap(_ value: UInt64) -> JsonReference {
        .init(.numb(number: .posInt(pos: UInt64(value))))
    }
    @inline(__always) fileprivate func wrap(_ value: String) -> JsonReference {
        .init(.str(string: value))
    }

    @inline(__always) fileprivate func wrap(_ float: Float) -> JsonReference {
        .init(.numb(number: .float(float: Double(float))))
    }

    @inline(__always) fileprivate func wrap(_ double: Double) -> JsonReference {
        .init(.numb(number: .float(float: double)))
    }

    fileprivate func wrap(
        _ date: Date, for codingPath: [CodingKey],
    ...
    fileprivate func wrap(
        _ data: Data, for codingPath: [CodingKey],
        _ additionalKey: (some CodingKey)? = AnyCodingKey?.none
    ) throws -> JsonReference {
    ...
    fileprivate func wrap(
        _ dict: [String: Encodable], for codingPath: [CodingKey],
        _ additionalKey: (some CodingKey)? = AnyCodingKey?.none
    ) throws -> JsonReference? {
        let depth = self.storage.count
        ...
        do {
            for (key, value) in dict {
                result.insert(
                    try wrap(value, for: rootPath, AnyCodingKey(stringValue: key)),
                    for: AnyCodingKey(stringValue: key))
            }
        } catch {
            // If the value pushed a container before throwing, pop it back off to restore state.
            if self.storage.count > depth {
    ...
    fileprivate func wrap(
        ...
        return try self.wrapGeneric(value, for: codingPath, additionalKey)
            ?? .init(.object(object: [:]))
    }

    fileprivate func wrapGeneric<T: Encodable>(
        _ value: T, for codingPath: [CodingKey],
        _ additionalKey: (some CodingKey)? = AnyCodingKey?.none
    ) throws -> JsonReference? {
        switch T.self {
        case is Date.Type:
    ...
    fileprivate func _wrapGeneric(
        _ encode: (__JsonEncoder) throws -> Void, for codingPath: [CodingKey],
        _ additionalKey: (some CodingKey)? = AnyCodingKey?.none
    ) throws -> JsonReference? {
...
private class __JsonReferencingEncoder: __JsonEncoder {
    ...
    init(
        referencing encoder: __JsonEncoder, at index: Int, codingPath: [CodingKey],
        wrapping ref: JsonReference
    ) {
        self.encoder = encoder
        self.reference = .array(ref, index)
        super.init(
            options: encoder.options, codingPath: codingPath + [AnyCodingKey(index: index)],
            codingPathDepth: codingPath.count + 1)
    }

    init(
        referencing encoder: __JsonEncoder, key: CodingKey, convertedKey: String,
        codingPath: [CodingKey], wrapping dictionary: JsonReference
    ) {
        self.encoder = encoder
        self.reference = .dictionary(dictionary, convertedKey)
        super.init(
            options: encoder.options, codingPath: codingPath + [key],
            codingPathDepth: codingPath.count + 1)
    }
...
open class JsonDecoder {
    ...
    open func decode<T: Decodable>(_ type: T.Type, from json: Json) throws -> T {
        let decoder = __JsonDecoder(
            userInfo: userInfo, from: json, codingPath: [], options: self.options)
...
final class __JsonDecoder: Decoder {
    ...
    init(
        userInfo: [CodingUserInfoKey: Any], from json: Json, codingPath: [CodingKey],
        options: JsonDecoder.Options
    ) {
    ...
    where Key: CodingKey {
        ...
        case let .object(object):
            return KeyedDecodingContainer(
                KeyedContainer<Key>(decoder: self, for: object, codingPath: codingPath))
        case .null:
...
extension __JsonDecoder {
    ...
    func unwrap<T: Decodable>(
        _ value: Json, as type: T.Type, for codingPath: [CodingKey],
        _ additionalKey: (some CodingKey)? = AnyCodingKey?.none
    ) throws -> T {
        if type == Date.self {
        ...
        if type == Decimal.self {
            return Decimal(try self.unwrap(value, as: Double.self, for: codingPath, additionalKey))
                as! T
        }
    ...
    func unwrapDate<K: CodingKey>(
        from value: Json, for codingPath: [CodingKey], _ additionalKey: K? = nil
    ) throws -> Date {
...
extension __JsonDecoder {
    struct KeyedContainer<Key: CodingKey>: KeyedDecodingContainerProtocol {
        ...
        func superDecoder() throws -> any Decoder {
            ...
            let decoder = __JsonDecoder(
                userInfo: self.decoder.userInfo, from: self.decoder.values.first!,
                codingPath: codingPath + [AnyCodingKey(stringValue: "super")],
        ...
        func superDecoder(forKey key: Key) throws -> any Decoder {
            ...
            let decoder = __JsonDecoder(
                userInfo: self.decoder.userInfo, from: self.decoder.values.first!,
                codingPath: codingPath + [key], options: self.decoder.options)
            decoder.values.append(value)
            return decoder
        }
    }
    ...
    struct UnkeyedContainer: UnkeyedDecodingContainer {
        ...
        mutating func decode(_ type: Bool.Type) throws -> Bool {
            ...
            guard case .bool(let bool) = value else {
                throw DecodingError.makeTypeMismatchError(
                    type: type, for: currentCodingPath, value: value)
            }
            advance()
            return bool
        }
...
extension DecodingError {
    fileprivate static func makeTypeMismatchError(
        type: Any.Type, for path: [CodingKey], value: Json
    ) -> DecodingError {
...
extension Document {
    ...
    public func on(
        ...
        let simple = SimpleHandler(callback, doc_change_callback)
